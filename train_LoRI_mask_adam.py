...
2025-11-17
不参数重置为 0
LoRI微调，并且在梯度更新前，将mask读入优化器，强制将 mask==0 的位置的梯度设为 
"""
import torch
import monai
...
from src.masked_adam import MaskedAdam
# ----------------- 基本配置 -----------------
num = 4
batch = 1
rank = 4
...
# ★ 在这里保存一份 Stage2 开始时的 B，用来事后对比（只是调试）
B_before_stage2 = [w_B.weight.detach().clone() for w_B in sam_lori.B_weights]
...
for epoch in range(num_epochs_stage2):
    ...
    for i, batch in enumerate(tqdm(train_dataloader)):
        ...
        # with torch.no_grad():
        #     for w_B, mask in zip(sam_lori.B_weights, sam_lori.lori_masks):
        #         if w_B.weight.grad is not None:
        #             w_B.weight.grad *= mask.to(w_B.weight.grad.device)
...
# ----------------- 事后验证：mask==0 的位置真的没更新吗？（可选调试） -----------------
with torch.no_grad():
    B_after = sam_lori.B_weights[0].weight.detach().cpu()
    B_before = B_before_stage2[0].cpu()
    mask = sam_lori.lori_masks[0].cpu().bool()

    diff_zero = (B_after[~mask] - B_before[~mask]).abs().max().item()
    diff_one  = (B_after[mask]  - B_before[mask]).abs().max().item()

print("Stage2 结束后检查：")
print("  mask==0 区域最大差值（理想应≈0）:", diff_zero)
print("  mask==1 区域最大差值（理想应>0）:",  diff_one)
