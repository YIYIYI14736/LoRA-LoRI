...
class LoRI_qkv(nn.Module):
    """
    LoRI adaption for attention modules. Only for queries and values
...
class LoRI_sam(nn.Module):
    """
    Class that takes the image encoder of SAM and add the lori weights to the attentions blocks

    Arguments:
        sam_model: Sam class of the segment anything model
        rank: Rank of the matrix for LoRI
        lori_layer: List of weights exisitng for LoRI
    ...
    def __init__(self, sam_model: Sam, rank: int, lori_layer=None):
        super(LoRI_sam, self).__init__()
        self.rank = rank
        assert rank > 0
        # base_vit_dim = sam_model.image_encoder.patch_embed.proj.out_channels

        if lori_layer:
            self.lori_layer = lori_layer
        else:
            # In each block, you have an attention block => total blocks -> nb lori layers
            self.lori_layer = list(range(len(sam_model.image_encoder.blocks)))
        ...
        for t_layer_i, blk in enumerate(sam_model.image_encoder.blocks):
            # if only lori on few layers
            if t_layer_i not in self.lori_layer:
                continue
        ...
        self.sam = sam_model
        self.lori_vit = sam_model.image_encoder


    def reset_parameters(self):
        """
        Initialize the LoRI A and B matrices like in the paper
        """
    ...
    def save_lori_parameters(self, filename: str):
        """
        Save the LoRI wieghts applied to the attention model as safetensors.
    ...
    def load_lori_parameters(self, filename: str):
        """
        Load a safetensor file of LoRI weights for the attention modules

        Arguments:
            filename: Name of the file containing the saved weights
        
        Return:
            None: Loads the weights to the LoRI_sam class
        """
